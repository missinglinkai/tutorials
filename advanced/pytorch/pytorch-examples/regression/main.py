...
with missinglink_project.create_experiment(
    fc,
    display_name='Linear Regression PyTorch',
    metrics={'Loss': F.smooth_l1_loss},
) as experiment:
    wrapped_loss = experiment.metrics['Loss']
    loss = 10
    for batch_idx in experiment.loop(condition=lambda _: loss >= 1e-3):
        # Get data
        ...
        # Forward pass
        output = wrapped_loss(fc(batch_x), batch_y)
        loss = output.data[0]
