...
def train_model(model, fields, optim, data_type, model_opt):
    ...
    with missinglink_project.create_experiment(
        model,
        display_name='OpenNMT Demo PyTorch',
        optimizer=optim.optimizer,
    ) as ml_experiment:

        trainer = onmt.Trainer(ml_experiment, model, train_loss, valid_loss, optim,
                               trunc_size, shard_size, data_type,
                               opt.normalization, opt.accum_count)

        for _, epoch in ml_experiment.epoch_loop(iterable=range(opt.start_epoch, opt.epochs + 1)):
            print('')

            # 1. Train for one epoch on the training set.
            train_loss.criterion.forward = ml_experiment.wrap_metrics({'Loss': train_loss.criterion.forward})['Loss']
            ...
            valid_loss.criterion.forward = ml_experiment.wrap_metrics({'Loss': valid_loss.criterion.forward})['Loss']
            with ml_experiment.validation():
