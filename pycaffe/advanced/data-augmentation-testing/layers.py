...
class customDataLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        self.num_classes = params['num_labels']
    ...
    def reshape(self, bottom, top):
        ...
        self.label = np.zeros((self.batch_size))
        for x in range(0, self.batch_size):
    ...
    def load_image(self, idx):
        ...
        im = Image.open(self.dir + '/image_' + idx + '.jpg')
    ...
    def random_crop(self, im):
        ...
        if width < self.crop_w + 3 + margin:
            im = im.resize((self.crop_w + 3 + margin, height), Image.ANTIALIAS)
            width, height = im.size
        if height < self.crop_h + 3 + margin:
            im = im.resize((width, self.crop_h + 3 + margin), Image.ANTIALIAS)
            width, height = im.size
